FOOTBALL PITCH MANAGEMENT & SOCIAL ECOSYSTEM


Project Overview
Project Name: Football Pitch Management System
Core Purpose: A dual-purpose platform for tracking/managing pitch rentals, additional services (tournaments), and a social network for matchmaking (finding opponents).
Target Audience: Sports enthusiasts, specifically football players.


Visual Style & Brand Identity
Mood & Tone: Minimalist & Clean. Focus on functionality by removing redundant details to ensure a clutter-free user experience.
Color Palette: Light Mode. Predominantly White and Light Gray, accented with a single primary color (e.g., Navy Blue or Forest Green).
Visual Elements: Abundant white space. Tables and Grids defined by very thin borders or subtle background color shifts.
Typography: Rounded Sans-serif fonts for high readability (e.g., Montserrat, Be Vietnam Pro).
Platform: Responsive design for both Desktop and Mobile.


Information Hierarchy
Priority 1: Booking Grid (Most prominent and intuitive component).
Priority 2: Matchmaking Feed (Located in Sidebar or immediately below the Dashboard to drive engagement).
Priority 3: Charts & Tables (Housed within "Statistics" or "History" tabs to maintain a clean homepage).


Key Navigation Structure (Sidebar Dashboard) A. User Module (Players)
Dashboard: Overview of upcoming matches, wallet balance, and new notifications.
Booking: Time-slot grid interface for pitch and time selection.
Matchmaking: Connection feed for opponent/team recruitment posts.
My Team: Member management, team fund, and match history.
My Activity: Personal stats and performance analysis charts.
B. Admin Module (Pitch Managers)
Admin Dashboard: Revenue reports (daily/monthly) and occupancy rate.
Schedule Management: Real-time view of all pitches, manual booking confirmation, or cancellations.
Pitch Management: Configuration of pitch types (5-a-side, 7-a-side, 11-a-side) and peak/off-peak pricing.
Customer CRM: User list, "No-show" history tracking, and loyalty rewards.
Finance: Revenue tracking for pitch rentals and add-on services (beverages, kit rentals).
C. Personal & Quick Access Tools
Personal Management: Wallet balance display, notification badges (match reminders/invites), and promotion/voucher vault.
Quick Widgets: Favorite/Frequent pitches dropdown and a direct support contact button.


Specialized Features & Interactive Flows
Interactive Booking Grid: A visual time-slot grid with pitches on the horizontal axis and hours on the vertical axis.
Color States: Green (Available), Red (Booked), Yellow (Pending Payment).
Drag-to-Book: Users can click and drag over a time slot to select duration. The UI responds with a translucent green highlight and real-time price estimation, ending with a quick-confirm popup.
Social Matchmaking Feed:
Cards: Mini cards displaying Skill Level (Amateur/Pro), Time, Location, and a "Join" button.
Interactions: "Join" or "Connect" buttons feature a Glow/Pulse effect. Statuses include "Waiting" or "Matched".
Data Visualization: Bar charts for frequency of play and tables for transaction history with date/status filters. giao diện dựa trên format Design.md này # Design System Inspired by Together AI
